(function($, window) {
	...
	proto.refresh = function(index, groupArray) {
		...
		this._loadItem(currentIndex, true);
			...
			$(this.element).on('longtap', 'img', function(e,tem) {
			//var  url=this.src;
			//var  suffix=this.src;
			var suffix = cutImageSuffix(this.src);
			var  imgUrl=this.src;
				mui.plusReady(function() {				
						...
						var downLoader =plus.downloader.createDownload(imgUrl, {
								...
								console.log('下载状dd态：' + status);
							});
							downLoader.start();
							
							/*plus.gallery.pick(function(url) {
										mui.toast("你选择了图片：" + url);
									}, function(error) {
										mui.toast("下载图片失败：" + url);
									}, {
...
//		this.empty();
	...
	$.getPreviewImage = function() {
		...
		/*mui(this).on('longtap','img',function(){
				mui.alert('334')
